...
class SentryMode:
  ...
  # Play sound function while offroad
  ...
  # take a snapshot, stich 360 image, save to disk, send alert to discord
  ...
  # Discord webhook function with/without image
  ...
  def update(self):
    ...
    if (t - self.transition_to_offroad_last) >= OFFROAD_DELAY * 0.5 and not self.played:
      self.played = True
      self._play_prebuilt_sound(ARMING_SOUND_FILE) # Warn driver of sentryd arming
    ...
    if self.armed:
      if delta > SENSITIVITY_THRESHOLD:
        ...
        if self.trigger_counter == 1:
          self.trigger_time = t
      # If movement detect, announce warning and prepare camera
      if self.trigger_counter == WARNING_TRIGGER_COUNT:
      ...
      # If triggers exceed threshold, trigger alarm and take snapshot
      if self.trigger_counter > MAX_TRIGGER_COUNT and t - self.trigger_time >= ALARM_TIME and not self.triggered_alarm:
      ...
      # Reset states after amount of time
      if self.trigger_counter > 0 and t - self.trigger_time >= RESET_TIME:
