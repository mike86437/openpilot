...
class SentryMode:
  ...
  def takeSnapshot(self):
    try:
      ...
      pic.save(f"{target_directory}back_image_{timestamp}.jpg") if pic else None
      fpic.save(f"{target_directory}front_image_{timestamp}.jpg") if fpic else None

      if pic and fpic:
        front_image = Image.open(f"{target_directory}front_image_{timestamp}.jpg")
        back_image = Image.open(f"{target_directory}back_image_{timestamp}.jpg")
        if front_image.height == back_image.height:
          ...
          result_image.save(f"{target_directory}360_image_{timestamp}.jpg")
          self.send_discord_webhook(ALERT_MESSAGE, f"{target_directory}360_image_{timestamp}.jpg")
        else:
  ...
  def update(self):
    ...
    if delta > SENSITIVITY_THRESHOLD:
      ...
      if self.secDelay >= 150:
        ...
        print("🚨 Triggered")
        self.secDelay = 0
        self.takeSnapshot() if self.frontAllowed else self.send_discord_webhook(ALERT_MESSAGE)
