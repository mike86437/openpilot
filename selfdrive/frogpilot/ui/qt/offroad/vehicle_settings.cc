...
QStringList getCarNames(const QString &carMake, QMap<QString, QString> &carModels) {
  ...
  QRegularExpression carModelRegex(R"delimiter((\w+)\s*=\s*(\w+PlatformConfig)\s*\((\s*\[[\s\S]*?\]\s*(?:,.*)?)\))delimiter");
  QRegularExpression carDocsRegex(R"delimiter((\w+)CarDocs\("([^"]+)")delimiter");
  ...
  while (carModelIt.hasNext()) {
    ...
    QString platformSection = carModelMatch.captured(3);
    ...
    while (carDocsIt.hasNext()) {
      ...
      QString carName = match.captured(2);
