...
class FrogPilotVCruise:
  ...
  def update(self, carState, controlsState, frogpilotCarState, frogpilotNavigation, gps_position, v_cruise, v_ego, frogpilot_toggles):
    ...
    if self.frogpilot_planner.lead_one.vLead + COMFORT_BRAKE < v_ego > CRUISING_SPEED and controlsState.state == State.enabled and self.frogpilot_planner.tracking_lead and frogpilot_toggles.human_following:
      if not self.frogpilot_planner.frogpilot_following.following_lead:
        decel_rate = (v_ego - self.frogpilot_planner.lead_one.vLead)**2 / (self.frogpilot_planner.lead_one.dRel * COMFORT_BRAKE)
        self.braking_target = v_ego - decel_rate
      else:
    ...
    if v_ego > CRUISING_SPEED and controlsState.state == State.enabled and frogpilot_toggles.map_turn_speed_controller:
      ...
      if frogpilot_toggles.speed_limit_controller:
        ...
        if self.override_slc:
          if frogpilot_toggles.speed_limit_controller_override_manual:
            ...
            self.overridden_speed = np.clip(self.overridden_speed, self.slc_target + self.slc_offset, v_cruise)
    ...
    if v_ego > CRUISING_SPEED and controlsState.state == State.enabled and self.frogpilot_planner.road_curvature_detected and frogpilot_toggles.vision_turn_speed_controller:
