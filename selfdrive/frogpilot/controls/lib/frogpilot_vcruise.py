...
class FrogPilotVCruise:
  ...
  def update(self, carControl, carState, controlsState, frogpilotCarControl, frogpilotCarState, frogpilotNavigation, gps_position, v_cruise, v_ego, frogpilot_toggles):
    ...
    if frogpilot_toggles.vision_turn_speed_controller and carControl.longActive and self.frogpilot_planner.road_curvature_detected and v_ego < 25:
    ...
    if self.vtsc_target >= v_cruise and self.mtsc_target >= v_cruise and v_ego > v_cruise:
    ...
    if frogpilot_toggles.force_standstill and carState.standstill and not self.override_force_stop and controlsState.enabled and not (self.frogpilot_planner.tracking_lead and lead.drel < 15):
