...
class ConditionalExperimentalMode:
  ...
  def update(self, carState, enabled, frogpilotNavigation, modelData, radarState, road_curvature, stop_distance, t_follow, v_ego):
    ...
    self.update_conditions(lead, lead_distance, modelData, road_curvature, standstill, stop_distance, t_follow, v_ego, v_lead)
  ...
  def update_conditions(self, lead, lead_distance, modelData, road_curvature, standstill, stop_distance, t_follow, v_ego, v_lead):
    self.lead_detection(lead)
    self.lead_slowing_down(lead, lead_distance, v_lead)
    self.road_curvature(road_curvature)
    self.slow_lead(lead, lead_distance, t_follow, v_ego)
    self.stop_sign_and_light(modelData, standstill, v_ego)
  ...
  def lead_slowing_down(self, lead, lead_distance, v_lead):
    if self.lead_detected:
      lead_close = lead_distance < CITY_SPEED_LIMIT
      lead_far = lead_distance >= CITY_SPEED_LIMIT and v_lead >= self.previous_v_lead
  ...
  def road_curvature(self, road_curvature):
    ...
    if lead_check and not self.red_light_detected:
      ...
      curve_active = 3.50 >= road_curvature > 1.1 and self.curve_detected
  ...
  def slow_lead(self, lead, lead_distance, t_follow, v_ego):
    if self.lead_detected:
  ...
  def stop_sign_and_light(self, modelData, standstill, v_ego):
    lead_check = self.stop_lights_lead or not self.lead_stopping or v_ego < CRUISING_SPEED
