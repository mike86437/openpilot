...
class ConditionalExperimentalMode:
  ...
  def stop_sign_and_light(self, lead_distance, lead_status, modelData, v_ego, v_lead, frogpilot_toggles):
    ...
    lead_stopping |= lead_status and (lead_distance < model_length or lead_distance < CITY_SPEED_LIMIT)

    model_stopped = model_length < ModelConstants.IDX_N
    model_stopping = model_length < v_ego * (ModelConstants.T_IDXS[ModelConstants.IDX_N - (5 if frogpilot_toggles.less_sensitive_lights else 3)])
