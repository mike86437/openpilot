...
from openpilot.selfdrive.frogpilot.frogpilot_variables import CITY_SPEED_LIMIT, State
...
class FrogPilotFollowing:
  ...
  def update(self, aEgo, controlsState, frogpilotCarState, lead_distance, v_ego, frogpilot_toggles):
    if controlsState.state == State.enabled and frogpilotCarState.trafficMode:
      if aEgo >= 0:
    ...
    elif controlsState.state == State.enabled:
      if aEgo >= 0:
    ...
    if controlsState.state == State.enabled and self.frogpilot_planner.tracking_lead:
