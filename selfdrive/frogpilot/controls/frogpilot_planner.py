...
class FrogPilotPlanner:
  ...
  def update_follow_values(self, jerk, lead_one, t_follow, trafficModeActive, v_ego, v_lead):
    ...
    if self.smoother_braking and v_lead < v_ego:
      distance_factor = np.maximum(1, lead_distance - (v_lead * t_follow))
      far_lead_offset = max((lead_distance - (v_ego * t_follow)) - stopping_distance, 0) if self.smoother_braking_far_lead and not trafficModeActive else 0
      braking_offset = np.clip((v_ego - v_lead) + far_lead_offset - COMFORT_BRAKE, 1, distance_factor)
      if self.smoother_braking_jerk and not trafficModeActive:
        jerk *= max(braking_offset**(1 / (COMFORT_BRAKE / t_follow)), 1)
  ...
  def update_v_cruise(self, carState, controlsState, enabled, liveLocationKalman, modelData, road_curvature, v_cruise, v_ego):
    ...
    filtered_targets = [target if target > CRUISING_SPEED else min(v_cruise, self.target25) for target in targets]
