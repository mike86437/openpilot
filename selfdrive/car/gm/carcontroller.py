        ...
        if not CC.longActive:
          ...
          if self.CP.carFingerprint in EV_CAR:
            ...
            if frogpilot_toggles.sport_plus:
              self.apply_gas = int(round(interp(accel if frogpilot_toggles.long_pitch else actuators.accel, self.params.EV_GAS_LOOKUP_BP_PLUS, self.params.GAS_LOOKUP_V_PLUS)))
            else:
              self.apply_gas = int(round(interp(accel if frogpilot_toggles.long_pitch else actuators.accel, self.params.EV_GAS_LOOKUP_BP, self.params.GAS_LOOKUP_V)))
            self.apply_brake = int(round(interp(brake_accel if frogpilot_toggles.long_pitch else actuators.accel, self.params.EV_BRAKE_LOOKUP_BP, self.params.BRAKE_LOOKUP_V)))
          else:
            if frogpilot_toggles.sport_plus:
              self.apply_gas = int(round(interp(accel if frogpilot_toggles.long_pitch else actuators.accel, self.params.GAS_LOOKUP_BP_PLUS, self.params.GAS_LOOKUP_V_PLUS)))
            else:
              self.apply_gas = int(round(interp(accel if frogpilot_toggles.long_pitch else actuators.accel, self.params.GAS_LOOKUP_BP, self.params.GAS_LOOKUP_V)))
            self.apply_brake = int(round(interp(brake_accel if frogpilot_toggles.long_pitch else actuators.accel, self.params.BRAKE_LOOKUP_BP, self.params.BRAKE_LOOKUP_V)))
