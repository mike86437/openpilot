...
ParamController(ConditionalExperimentalModeSpeed, "ConditionalExperimentalModeSpeed", "Below", "Switch to 'Experimental Mode' below this speed when there is no lead vehicle.", "../assets/offroad/icon_blank.png",
  const int speed = params.getInt("ConditionalExperimentalModeSpeed");
  return speed == 0 ? "Off" : QString::number(speed) + (isMetric ? " kph" : " mph");,
  return std::clamp(v, 0, isMetric ? 150 : 99);
)

ParamController(ConditionalExperimentalModeSpeedLead, "ConditionalExperimentalModeSpeedLead", "With Lead", "Switch to 'Experimental Mode' below this speed when there is a lead vehicle.", "../assets/offroad/icon_blank.png",
  const int speedLead = params.getInt("ConditionalExperimentalModeSpeedLead");
