...
void AnnotatedCameraWidget::updateState(const UIState &s, const FrogPilotUIState &fs) {
  ...
  const bool nav_alive = sm.alive("navInstruction") && sm["navInstruction"].getValid() || fs.frogpilot_toggles.value("show_speed_limits").toBool() || fs.frogpilot_toggles.value("speed_limit_controller").toBool();
  ...
  if (fs.frogpilot_toggles.value("show_speed_limits").toBool() || fs.frogpilot_toggles.value("speed_limit_controller").toBool()) {
    speedLimit = fs.frogpilot_scene.speed_limit;
  } else {
