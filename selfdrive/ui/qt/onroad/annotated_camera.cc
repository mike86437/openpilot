...
void AnnotatedCameraWidget::drawHud(QPainter &p) {
  ...
  if (!(bigMapOpen || hideSpeed)) {
    if (standstillDuration > 1) {
      ...
      int shiftX = (QDateTime::currentSecsSinceEpoch() / 100) % 21 - 10;  // Shifts between -2, -1, 0, +1, +2 pixels
      int shiftY = (QDateTime::currentSecsSinceEpoch() / 150) % 21 - 10; // Shifts every 2 minutes
      p.setFont(InterFont(176, QFont::Bold));
