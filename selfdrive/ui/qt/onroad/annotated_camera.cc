...
void AnnotatedCameraWidget::drawHud(QPainter &p) {
  ...
  if (!(bigMapOpen || hideSpeed)) {
    if (standstillDuration > 1) {
      ...
      int shiftX = (QDateTime::currentSecsSinceEpoch() / 1000) % 21 - 10;  // Shifts between -2, -1, 0, +1, +2 pixels
      int shiftY = (QDateTime::currentSecsSinceEpoch() / 1500) % 21 - 10; // Shifts every 2 minutes
      p.setFont(InterFont(176, QFont::Bold));
