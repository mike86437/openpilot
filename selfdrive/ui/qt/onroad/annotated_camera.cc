...
void AnnotatedCameraWidget::drawLaneLines(QPainter &painter, const UIState *s, const float v_ego) {
  ...
  if (scene.show_stopping_point) {
    ...
    bool following_lead = scene.has_lead && scene.lead_distance < scene.model_length;
    ...
    if (model_stopping && !curve_detected && !following_lead) {
      ...
      QPointF adjusted_point = last_point - QPointF(stopSignImg.width() / 2, stopSignImg.height()) + QPointF(scene.model_length * 0.1, 0);
      ...
      QPointF text_position = last_point - QPointF(text_width / 2, stopSignImg.height() + 35) + QPointF(scene.model_length * 0.1, 0);
