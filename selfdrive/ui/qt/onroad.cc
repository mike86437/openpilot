...
void AnnotatedCameraWidget::updateState(const UIState &s) {
  ...
  if (map_settings_btn->isEnabled()) {
    ...
    main_layout->setAlignment(map_settings_btn, (flip_side ? Qt::AlignLeft : Qt::AlignRight) | (move_up ? Qt::AlignCenter : Qt::AlignBottom));
  }
  ...
  frogSignals = s.scene.frog_signals;
...
void AnnotatedCameraWidget::drawDeveloperUI(QPainter &p) {
  ...
  const auto timeDiff = std::chrono::duration_cast<std::chrono::seconds>(now - lastUpdated).count();

  // Create the insights
  const auto &currUnits = mapOpen ? units[2] : units[1];
  const double displayAcceleration = (((timeDiff >= 5) ? currentAcceleration : maxAcceleration) * conversions[0][developerUI]);
  const QString speedMetric = QString::fromUtf8(units[0][developerUI].data());

  const QString maxAccText = QString("Accel: %1%2%3").arg(displayAcceleration, 0, 'f', 2)
                                  .arg(speedMetric)
                                  .arg((timeDiff >= 5 || mapOpen) ? "" : QString(" - Max: %1%2").arg(maxAcceleration * conversions[0][developerUI], 0, 'f', 2).arg(speedMetric));

  // Define the different segments of the insightsText
  const auto createDiffText = [&](const double data, const double stockData) {
    return (data != stockData) ? QString(" (%1%2)").arg(data - stockData > 0 ? "+" : "").arg(data - stockData) : QString();
  };

  const QString obstacleDiff = createDiffText(obstacleDistance, obstacleDistanceStock);
  const QString stopDiff = createDiffText(stoppedEquivalence, stoppedEquivalenceStock);

  const auto createText = [&](const QString &title, const double data) {
    return title + QString::number(data * conversions[1][developerUI], 'f', 0);
  };
  ...
  const auto textWidth = [&](const QString &text) { return p.fontMetrics().width(text); };

  // Calculate the total width of all the text segments
  const QString fullText = maxAccText 
                         + " | " + obstacleText + (obstacleDiff.isEmpty() ? "" : obstacleDiff + " " + QString::fromUtf8(currUnits[developerUI].data())) 
                         + " - " + stopText + (stopDiff.isEmpty() ? "" : stopDiff + " " + QString::fromUtf8(currUnits[developerUI].data())) 
                         + " = " + followText + " " + QString::fromUtf8(currUnits[developerUI].data());

  // Calculate the total width of the full text
  const int totalTextWidth = p.fontMetrics().width(fullText);

  // Calculate the start position to center all the text segments in the available space
  ...
  const auto drawText = [&](const QString &text, const QColor color) {
    ...
    textStartPos += textWidth(text);
  };

  // Draw maxAccText with appropriate color
  drawText(maxAccText, timeDiff < 5 ? Qt::red : Qt::white);

  // Draw the obstacleText
  drawText(obstacleText, Qt::white);
  if (!obstacleDiff.isEmpty()) drawText(obstacleDiff, (obstacleDistance - obstacleDistanceStock) > 0 ? Qt::green : Qt::red);
  drawText(QString::fromUtf8(currUnits[developerUI].data()), Qt::white);

  // Draw the stopText
  drawText(stopText, Qt::white);
  if (!stopDiff.isEmpty()) drawText(stopDiff, (stoppedEquivalence - stoppedEquivalenceStock) > 0 ? Qt::green : Qt::red);
  drawText(QString::fromUtf8(currUnits[developerUI].data()), Qt::white);
