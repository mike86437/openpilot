...
void ExperimentalButton::paintEvent(QPaintEvent *event) {
  ...
  if (!scene.show_driver_camera) {
    if (rotatingWheel) {
      drawIconRotate(p, QPoint(btn_size / 2, btn_size / 2), img, background_color, (isDown() || (!engageable && !scene.always_on_lateral_active)) ? 0.6 : 1.0, steeringAngleDeg);
    } else {
      drawIcon(p, QPoint(btn_size / 2, btn_size / 2), img, background_color, (isDown() || (!engageable && !scene.always_on_lateral_active)) ? 0.6 : 1.0);
    }
  }
}
