...
class LongitudinalPlanner:
  def __init__(self, CP, init_v=0.0, init_a=0.0):
    ...
    self.limit = self.params.get_int("ConditionalExperimentalModeSpeed") * (CV.KPH_TO_MS if self.is_metric else CV.MPH_TO_MS)
    self.limit_lead = self.params.get_int("ConditionalExperimentalModeSpeedLead") * (CV.KPH_TO_MS if self.is_metric else CV.MPH_TO_MS)
  ...
  def check_conditions(self, sm, v_ego, v_lead):
    ...
    stop_light_detected = self.stop_sign_and_light(carstate, lead, lead_distance, modeldata, radarstate, v_ego, v_lead) and self.stop_lights and not standstill
    if stop_light_detected and self.stop_lights:
      self.status_value = 7
      return True

    # Road curvature check - Need to check for stop lights/stop signs since the curve function also detects them
    self.curve = self.road_curvature(lead, modeldata, v_ego) and self.curves and not standstill
    if self.curve:
      self.status_value = 8
      return True
  ...
  def road_curvature(self, lead, modeldata, v_ego):
    ...
    if self.curves_lead or not lead:
      ...
      if len(predicted_lateral_accelerations) == len(predicted_velocities) != 0:
        ...
        if 5.0 > curvature >= 1.6 or (self.curve and 5.0 > curvature >= 1.1):
