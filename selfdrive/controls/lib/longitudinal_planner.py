  ...
  def update(self, sm):
    ...
    if reset_state:
      ...
      if sm['radarState'].leadOne.status:
        max_acceleration = accel_limits[1]
      else:
        # Slowly ramp up the acceleration to prevent inviting other drivers to a street race
    ...
    self.mpc.update(sm['radarState'], sm['frogpilotPlan'].vCruise, x, v, a, j, sm['frogpilotPlan'].tFollow, sm['frogpilotCarControl'].trafficModeActive, personality=sm['controlsState'].personality)
