...
METADATA_PATH = Path(MODELS_PATH) / f'{MODEL_NAME}_metadata.pkl' if (Path(MODELS_PATH) / f'{MODEL_NAME}_metadata.pkl').exists() else Path(__file__).parent / 'models/supercombo_metadata.pkl'
...
class ModelState:
  ...
  def run(self, buf: VisionBuf, wbuf: VisionBuf, transform: np.ndarray, transform_wide: np.ndarray,
    ...
    if SECRET_GOOD_OPENPILOT:
      ...
      if wbuf is not None:
        new_big_img = self.wide_frame.prepareSecret(wbuf, transform.flatten(), self.model.getCLBuffer("big_input_imgs"))
