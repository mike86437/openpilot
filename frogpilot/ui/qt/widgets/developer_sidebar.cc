...
void DeveloperSidebar::updateState(const UIState &s, const FrogPilotUIState &fs) {
  ...
  const cereal::RadarState::Reader &radarState = fpsm["radarState"].getRadarState();
  ...
  dangerJerkStatus = ItemStatus(QPair<QString, QString>(tr("vLead"), QString::number(radarState.getLeadOne().getVLead(), 'f', 2)), metricColor);
  ...
  latAccelStatus = ItemStatus(QPair<QString, QString>(tr("LAT ACCEL"), QString::number(controlsState.getCurvature() * carState.getVEgo() * carState.getVEgo() - liveParameters.getRoll() * 9.81, 'f', 2)), metricColor);
  ...
  speedJerkStatus = ItemStatus(QPair<QString, QString>(tr("dRel"), QString::number(radarState.getLeadOne().getDRel(), 'f', 2)), metricColor);
  steerAngleStatus = ItemStatus(QPair<QString, QString>(tr("tFollowing"), QString::number(radarState.getLeadOne().getDRel() / std::max(carState.getVEgo(), 1.0f), 'f', 2)), metricColor);
  ...
  stiffnessFactorStatus = ItemStatus(QPair<QString, QString>(tr("vEgo"), QString::number(carState.getVEgo(), 'f', 2)), metricColor);
