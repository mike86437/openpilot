...
class FrogPilotVCruise:
  ...
  def update(self, gps_position, v_cruise, v_ego, sm, frogpilot_toggles):
    ...
    if v_ego > CRUISING_SPEED and sm["controlsState"].enabled and frogpilot_toggles.map_turn_speed_controller:
      ...
      if (v_lead + min(dFollow, 3)) < v_ego > CRUISING_SPEED and self.frogpilot_planner.tracking_lead:
        mtsc_active = True
    ...
    if sm["carState"].standstill and not self.override_force_stop and sm["controlsState"].enabled and frogpilot_toggles.force_standstill and not (self.frogpilot_planner.tracking_lead and getattr(self.frogpilot_planner.lead_one, "dRel", float("inf")) < 15):
